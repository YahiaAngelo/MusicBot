...
class MusicBot(discord.Client):
    ...
    async def _get_anghami_playlist_songs(self, url):
        playlist = []
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as r:
                html = await r.text()
                soup = BeautifulSoup(html, "html.parser")
                text = soup.find_all('script', type='application/ld+json')[-1].string
                data = json.loads(text)
                for track in data['track']:
                    playlist.append(track['byArtist']['name'] + ' ' + track['name'])
                return playlist
    ...
    async def cmd_play(self, message, player, channel, author, permissions, leftover_args, song_url):
        ...
        if "play.anghami.com" in song_url:
            tracks = await self._get_anghami_playlist_songs(song_url)
